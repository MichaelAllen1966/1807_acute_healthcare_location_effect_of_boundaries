...
class GlobVariables():
    ...
    def __init__(self):
        ...
        self.pareto_scores_used = [0, 1, 2, 3, 4, 5, 6, 7]
        self.output_location = 'test'
        self.target_travel = 30  # target distance/time
        self.target_min_admissions = 600  # target sustainable admissions
        self.maximum_generations = 1  # generations of genetic algorithm
        self.fix_hospital_status = True  # Allows hospitals to be forced open or closed
        self.initial_random_population_size = 0
        ...
        self.score_initial_population_without_selection = True
...
class Data():
    ...
    def identify_region(self):
        # Set use_admissions_region to true if admissions data contains regions
        # If set to false model will assign reagion to each patient location based on region of closest hospital
...
class Master():
    ...
    def initialise_population(self):
        ...
        if len(self.data.loaded_population) > 0:
            ...
            if self.global_vars.score_initial_population_without_selection == False:
                if self.global_vars.score_initial_population_without_selection:
                    self.pop.population = np.unique(self.pop.population, axis=0)
                # Fix status if necessary
                if self.global_vars.fix_hospital_status:
                    self.pop.population = self.pop.fix_hospital_status(self.data.hospitals, self.pop.population)
    ...
    def select_population(self, generation):
        ...
        # When option selected, score first population without selection
        if self.global_vars.score_initial_population_without_selection and generation == 0:
            if self.pop.population.shape[0] < 20000:
                hamming = np.average(pdist(self.pop.population, 'hamming'))
            else:
                hamming = 999
            ...
            self.save_pareto(self.score.scores, self.pop.population, self.score.hospital_admissions)
            pareto_size = self.pop.population.shape[0]
            return (self.pop.population, pareto_size)
            ...
            while rows_required > 0:
                ...
                for i in range(rows_required):
                    ...
                    for key, values in regions_dictionary.items():
                        ...
                        new_random_population[i, int(values[pick_index])] = 1
                if fix_hospitals:
